...
static void get_screen_size( unsigned *width, unsigned *height )
{
	...
	*height = Machine->debug_bitmap->height / Machine->debugger_font->height - 2;
}
...
{	(1<<EDIT_CMDS), "A",            0,          CODE_NONE, "[<update>]", "Animate (trace) and update display once per frame [or every <update> opcodes only]", cmd_animate },
{	(1<<EDIT_CMDS), "D",            0,          CODE_NONE, "<1|2> <address>", "Display memory <1|2> starting at <address>", cmd_display_memory },
{	(1<<EDIT_CMDS), "E",            0,          CODE_NONE, "<1|2> [<address>]", "Edit memory window <1|2> [at <address>]", cmd_edit_memory },
{	(1<<EDIT_CMDS), "M",            0,          CODE_NONE, "<1|2> [BYTE|WORD|DWORD]", "Change memory window mode to default [to BYTE|WORD|DWORD (or 0|1|2)]", cmd_set_memory_mode },
{	(1<<EDIT_CMDS), "F",            0,          CODE_NONE, "", "Fast", cmd_fast },
{	(1<<EDIT_CMDS), "G",            0,          CODE_NONE, "[<address>]", "Go [and break at <address>]", cmd_go_break },
{	(1<<EDIT_CMDS), "J",            0,          CODE_NONE, "<address>", "Jump to <address> in disassembly window", cmd_jump },
{	(1<<EDIT_CMDS), "R",            0,          CODE_NONE, "<register> <value>", "Replace <register> with <value> (<value> may also be a <register>)", cmd_replace_register },
{	(1<<EDIT_CMDS), "BP",           "BPX",      CODE_NONE, "<address> [<times>]", "Break on execution of <address> [after ignoring it <times>]", cmd_brk_exec_set },
{	(1<<EDIT_CMDS), "BC",           0,          CODE_NONE, "", "Clear execution breakpoint", cmd_brk_exec_clear },
{	(1<<EDIT_CMDS), "RP",           0,          CODE_NONE, "<register> [<value> [<mask>]]", "Break if <register> changes [to <value> [compare after applying <mask>]]", cmd_brk_regs_set },
{	(1<<EDIT_CMDS), "RC",           0,          CODE_NONE, "", "Clear register watchpoint", cmd_brk_regs_clear },
{	(1<<EDIT_CMDS), "WP",           "BPW",      CODE_NONE, "<address> [<value>]", "Break if data at <address> changes [to <value>]", cmd_brk_data_set },
{	(1<<EDIT_CMDS), "WC",           0,          CODE_NONE, "", "Clear data watchpoint", cmd_brk_data_clear },
#if defined(PINMAME) && defined(DBG_BPR)
{	(1<<EDIT_CMDS), "BPR",          0,      CODE_NONE, "<start> [<end>]", "Break if data between <start> and <end> is read or written", bpr_set },
#endif /* DBG_BPR */
{	(1<<EDIT_CMDS), "HERE",         0,          CODE_NONE, "", "Run to cursor", cmd_here },
{	(1<<EDIT_CMDS), "DASM",         0,          CODE_NONE, "<filename> <start> <end> [<boolean>]", "Disassemble to <filename> from address <start> to <end>\n" \
"Opcode dump on by default [OFF|NO|0 without]", cmd_dasm_to_file },
{	(1<<EDIT_CMDS), "DUMP",         0,          CODE_NONE, "<filename> <start> <end> [<data size> [<ASCII mode> [<prog/data memory>]]]", "Dump to <filename> from address <start> to <end>\n" \
...
{	(1<<EDIT_CMDS), "TRACE",        0,          CODE_NONE, "{<filename> [<reg1> [<reg2>...]]}|OFF", "Trace to <filename> [dumping <reg1> [<reg2>...]] | OFF to stop tracing.", cmd_trace_to_file },
#ifdef PINMAME
/* CODELIST */
{	(1<<EDIT_CMDS), "CODELIST",      0,          CODE_NONE, "[filename]|OFF", "Collect code addresses during execution and condense them to blocks.\nCode blocks data is written to <filename> when switched off", cmd_codelist_to_file },
#endif /* PINMAME */
{	(1<<EDIT_CMDS), "SAVE",         0,          CODE_NONE, "<filename> <start> <end> [OPCODES|DATA]", "Save binary to <filename> from address <start> to <end>\n" \
	"[either OPCODES (from OP_ROM, default) or DATA (from OP_RAM), also 0|1].", cmd_save_to_file },
{	(1<<EDIT_CMDS), "SCANLINE",     0,          CODE_NONE, "", "Toggles the display of scanlines", cmd_toggle_scanlines },
#ifdef PINMAME
{	(1<<EDIT_CMDS), "SD",     0,          CODE_NONE, "", "Disables Sound when debugger window is active", cmd_sound_disable },
{	(1<<EDIT_CMDS), "SE",     0,          CODE_NONE, "", "Enables Sound when debugger window is active", cmd_sound_enable },
#endif
{	(1<<EDIT_CMDS), "IGNORE",       0,          CODE_NONE, "<cpunum>", "Ignore CPU #<cpunum> while debugging or tracing", cmd_set_ignore },
{	(1<<EDIT_CMDS), "OBSERVE",      0,          CODE_NONE, "<cpunum>", "Observe CPU #<cpunum> while debugging or tracing", cmd_set_observe },
{	(1<<EDIT_CMDS), "REPEAT",       0,          CODE_NONE, "rate", "Set keyboard initial repeat rate (rate/frame will increase to 1/frame)", cmd_set_key_repeat },
{	(1<<EDIT_CMDS), "CASE",         0,          CODE_NONE, "DEFAULT|LOWER|UPPER (also 0|1|2)", "Set disassembly case style.", cmd_set_dasm_case },
{	(1<<EDIT_CMDS), "OPCODES",      0,          CODE_NONE, "<boolean>", "Display opcodes in disassembly window", cmd_set_dasm_opcodes },
{	(1<<EDIT_CMDS), "RELATIVE",     0,          CODE_NONE, "<boolean>", "Display relative jump addresses in disassembly window", cmd_set_dasm_relative_jumps },
{	(1<<EDIT_CMDS), "SQUEEZE",      0,        CODE_NONE, "<boolean>", "Allow squeezed memory display", cmd_set_mem_squeezed },
{	(1<<EDIT_CMDS), "COLOR",       0,          CODE_NONE, "<element> <foreground> [<background>]", "Set <element> color to <foreground> on BLACK [or <background>].\nFor a list of <elements> and <colors> see mamedbg.cfg", cmd_set_element_color },
{	(1<<EDIT_CMDS)|(1<<EDIT_DASM), 0,				0,			KEYCODE_UP, "", "Move cursor up in disassembly window", cmd_dasm_up },
{	(1<<EDIT_CMDS)|(1<<EDIT_DASM), 0,				0,			KEYCODE_DOWN, "", "Move cursor down in disassembly window", cmd_dasm_down },
{	(1<<EDIT_CMDS)|(1<<EDIT_DASM), 0,				0,			KEYCODE_PGUP, "", "Move cursor up one page in disassembly window", cmd_dasm_page_up },
{	(1<<EDIT_CMDS)|(1<<EDIT_DASM), 0,				0,			KEYCODE_PGDN, "", "Move cursor down one page in disassembly window", cmd_dasm_page_down },
{	(1<<EDIT_CMDS)|(1<<EDIT_DASM), 0,				0,			KEYCODE_HOME, "", "Move cursor to first page in disassembly window", cmd_dasm_home },
{	(1<<EDIT_CMDS)|(1<<EDIT_DASM), 0,				0,			KEYCODE_END, "", "Move cursor to last page in disassembly window", cmd_dasm_end },
{	(1<<EDIT_CMDS)|(1<<EDIT_DASM), 0,				0,			KEYCODE_LEFT, "", "Back to the previous point in 'follow history'", cmd_dasm_hist_back },
{	(1<<EDIT_CMDS)|(1<<EDIT_DASM), 0,				0,			KEYCODE_RIGHT, "", "Follow the current instruction's code or data reference", cmd_dasm_hist_follow },
{	ALL, 0,				0,			KEYCODE_TAB, "", "Switch between windows (backwards SHIFT+TAB)", cmd_switch_window },
{	(1<<EDIT_DASM), 0,				0,			KEYCODE_D, "", "Change disassembly case style to default", NULL },
{	(1<<EDIT_DASM), 0,				0,			KEYCODE_L, "", "Change disassembly case style to lower case", NULL },
{	(1<<EDIT_DASM), 0,				0,			KEYCODE_U, "", "Change disassembly case style to upper case", NULL },
{	(1<<EDIT_DASM), 0,				0,			KEYCODE_M, "", "Toggle disassembly opcode display mode", NULL },
{	(1<<EDIT_MEM1)|(1<<EDIT_MEM2), 0,				0,			KEYCODE_H, "", "Toggle between hex, ASCII and full character set mode", NULL },
{	(1<<EDIT_MEM1)|(1<<EDIT_MEM2), 0,				0,			KEYCODE_P, "", "Toggle memory display between DATA and PROGRAM memory (Harvard-architecture CPUs)", NULL },
{	(1<<EDIT_MEM1)|(1<<EDIT_MEM2), 0,				0,			KEYCODE_I, "", "Toggle memory display between CPU internal and normal memory", NULL },
{	(1<<EDIT_MEM1)|(1<<EDIT_MEM2), 0,				0,			KEYCODE_M, "", "Switch memory display mode between bytes, words and dwords", NULL },
{	(1<<EDIT_MEM1)|(1<<EDIT_MEM2), 0,				0,			KEYCODE_S, "", "Search memory for a sequence of bytes", cmd_search_memory },
{	ALL, 0,				0,			KEYCODE_F1, "", "Help - maybe you realized this ;)", cmd_help },
{	(1<<EDIT_CMDS)|(1<<EDIT_DASM), 0,				0,			KEYCODE_F2, "", "Toggle breakpoint at current cursor position", cmd_brk_exec_toggle },
{	(1<<EDIT_CMDS)|(1<<EDIT_DASM), 0,				0,			KEYCODE_F4, "", "Run to cursor", cmd_run_to_cursor },
{	(1<<EDIT_MEM1)|(1<<EDIT_MEM2), 0,				0,			KEYCODE_F4, "", "Set data watchpoint to current memory location", cmd_brk_data_toggle },
{	ALL, 0,				0,			KEYCODE_F6, "", "Set the focus to the next (not ignored) CPU", cmd_focus_next_cpu },
{	ALL, 0,				0,			KEYCODE_F8, "", "Step one instruction", cmd_step },
{	ALL, 0,				0,			KEYCODE_F9, "", "Animate (trace) at speed set by last \"A\" command", cmd_animate },
{	ALL, 0,				0,			KEYCODE_F10, "", "Step over instruction at cursor (ie. execute call, jsr or bsr)", cmd_step_over },
{	ALL, 0,				0,			KEYCODE_F12, "", "Go!", cmd_go },
{	ALL, 0,				0,			KEYCODE_ESC, "", "Go!", cmd_go },
#ifdef PINMAME
{	ALL, 0,              0,          KEYCODE_ENTER, "", "\r[SHIFT+ENTER]\tLike F10, but also skips loops, function calls, etc.", cmd_jumpover },
#endif
